...
When deployable app assets change, bump `version.js` so the service worker receives a distinct cache. Activation removes old caches, but this affects only cache storage—not localStorage game data.
